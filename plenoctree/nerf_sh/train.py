...
# JAX compatibility fix for newer JAX versions
if not hasattr(jax, 'api'):
    class MockJaxApi:
        def __getattr__(self, name):
            if name == 'device_put_sharded':
                return getattr(jax, 'device_put_sharded', None)
            raise AttributeError(f"module 'jax.api' has no attribute '{name}'")
    jax.api = MockJaxApi()
...
def main(unused_argv):
    ...
    # Initialize JSON logger for comprehensive metrics tracking
    metrics_logger = create_logger(FLAGS.train_dir, "nerf_training_metrics.json")
    
    # Initialize memory tracker for memory efficiency analysis
    memory_tracker = create_memory_tracker()
    h0print('* Memory tracker initialized')
    
    # Capture baseline memory before training starts
    baseline_snapshot = memory_tracker.capture_snapshot(step=0, timestamp=None)
    ...
    for step, batch in zip(range(init_step, FLAGS.max_steps + 1), pdataset):
        ...
        if jax.host_id() == 0:
            if step % FLAGS.print_every == 0:
                ...
                # Log training metrics to JSON every print_every steps (same frequency as console output)
                timing_info = {
                    "steps_per_sec": steps_per_sec,
                    "rays_per_sec": rays_per_sec,
                    "avg_loss": avg_loss,
                    "avg_psnr": avg_psnr,
                    "time_interval": FLAGS.print_every / steps_per_sec
                }
                
                # Capture memory snapshot and calculate efficiency indices
                memory_snapshot = memory_tracker.capture_snapshot(step)
                memory_metrics = memory_tracker.get_memory_metrics(memory_snapshot)
                
                # Calculate memory efficiency indices using current PSNR
                current_psnr = float(stats.psnr[0]) if hasattr(stats.psnr, '__getitem__') else float(stats.psnr)
                efficiency_indices = memory_tracker.calculate_efficiency_indices(
                    psnr=current_psnr,
                    snapshot=memory_snapshot
                )
                
                metrics_logger.log_training_step(step, stats, lr, timing_info, memory_metrics, efficiency_indices)
        ...
        if FLAGS.render_every > 0 and step % FLAGS.render_every == 0:
            ...
            if jax.host_id() == 0:
                ...
                # Log evaluation metrics to JSON with memory tracking
                eval_timing_info = {
                    "eval_time": eval_time,
                    "eval_rays_per_sec": float(rays_per_sec),
                    "num_rays": int(num_rays)
                }
                
                # Capture memory snapshot during evaluation
                eval_memory_snapshot = memory_tracker.capture_snapshot(step)
                eval_memory_metrics = memory_tracker.get_memory_metrics(eval_memory_snapshot)
                
                # Calculate memory efficiency indices for evaluation
                eval_efficiency_indices = memory_tracker.calculate_efficiency_indices(
                    psnr=float(psnr),
                    ssim=float(ssim),
                    snapshot=eval_memory_snapshot
                )
                
                metrics_logger.log_evaluation_step(
                    step, psnr, ssim, eval_timing_info, 
                    memory_metrics=eval_memory_metrics,
                    efficiency_indices=eval_efficiency_indices
                )
